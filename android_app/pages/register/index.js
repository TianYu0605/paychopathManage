...
const Register = ({navigation}) => {
  ...
  const register = () => {
    navigation.navigate('UsernameAndPasswordLogin')
  }
  const sendCode = () => {
  ...
  return (
    <View style={[styles.container,{width,height}]}>
      ...
      <View style={[styles.flexRowStart,{width:width*.8,borderBottomWidth:1,height:40,borderBottomColor:'#CECECE',marginTop:20}]}>
        <Fontisto name='calculator' color='#4CC596' size={16} />
        <TextInput 
          style={[styles.inputWrapper, {width:width*.6,height: width*.08}]} 
      ...
      <View style={[styles.flexRowStart,{width:width*.8,borderBottomWidth:1,height:40,borderBottomColor:'#CECECE',marginTop:20}]}>
        ...
        <TextInput 
          style={[styles.inputWrapper, {width:width*.6,height: width*.08}]} 
          onChangeText={passwordAgainChange} 
          placeholder="请再次输入密码"
          value={passwordAgain}
          clearButtonMode="while-editing"
        />
      ...
      <TouchableNativeFeedback>
        <View style={[styles.flexCenter,{width:width*.95,height:45,borderRadius:5,marginTop:50,backgroundColor:'#4CC596'}]} onTouchEnd={()=>register()}>
          <Text style={{fontSize:setSpText(15),color:'#ffffff'}}>注册</Text>
        </View>
      </TouchableNativeFeedback>
    </View>
  );
